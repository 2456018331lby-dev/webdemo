...
- Settings center with local preferences, notification rules, notification inbox, push readiness checks, local backup export/restore, device maintenance
...
2. Persist `/settings` device maintenance edits in local storage and include them in local backup/restore.
