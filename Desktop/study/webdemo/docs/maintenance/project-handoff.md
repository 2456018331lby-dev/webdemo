...
29. 设备控制 pending 反馈：前端现在区分 simulator ack、busy retry 和 `SMART_HOME_COMMAND_DELIVERY=polling`；真实轮询模式下控制卡显示“已排队 / 等待硬件确认”，锁定按钮，并保持 reported relay state 不被提前改写
30. 设备命令生命周期反馈：控制卡按 `commandId` 接收最新命令状态；`delivered` 显示“已送达”但继续等待硬件 ack，`failed` / `timed_out` 会解除本地等待锁并提示失败原因
31. 命令历史生命周期语义收敛：命令历史摘要不再把 `delivered` 当成功态展示，而是使用 in-flight tone 提醒“已送达但仍等待硬件确认”；设备详情页页面级 inline style 已迁移为 CSS class
32. 设置页危险操作确认优化：设备恢复出厂不再使用原生 `window.confirm`，改为应用内确认对话框，桌面/移动端都保持统一视觉、可取消、可验证，并继续把设备恢复为“离线 / 未分配”
...
- 2026-06-09 设备控制 pending 反馈复核：`npm test -- --run apps/web/src/components/device-control-panel.test.tsx apps/web/src/components/device-command-client.test.tsx` 2 files / 12 tests 通过；随后全量 `npm test -- --run` 24 files / 110 tests 通过；`npm run lint` 和 `npm run build` 通过；临时 `SMART_HOME_COMMAND_DELIVERY=polling` production smoke 验证 `/devices/device-relay-01` 显示“等待硬件确认 / ESP32S3 轮询队列”，按钮锁定，hero reported state 保持不变，桌面 `scrollWidth=1280`、移动端 `scrollWidth=393`；build 后已删除 `apps/web/.next/`
- 2026-06-09 设备命令生命周期反馈复核：`npm test -- --run apps/web/src/components/device-control-panel.test.tsx apps/web/src/components/device-command-client.test.tsx` 2 files / 14 tests 通过；随后全量 `npm test -- --run` 24 files / 112 tests 通过；`npm run lint` 和 `npm run build` 通过；真实 polling production smoke 使用 `SMART_HOME_COMMAND_DELIVERY=polling` 与临时设备 token，桌面 `1366x900`、移动端 `390x844` 均验证从“已排队”刷新到“已送达”，按钮保持“等待确认”禁用；本轮清理 ignored `.omx/` 和 build 后 `apps/web/.next/`，未保留截图、trace、报告或临时脚本
- 2026-06-09 命令历史生命周期语义复核：`npm test -- --run apps/web/src/components/command-history-list.test.tsx apps/web/src/components/device-command-client.test.tsx` 2 files / 9 tests 通过；随后全量 `npm test -- --run` 24 files / 113 tests 通过；`npm run lint` 和 `npm run build` 通过；真实 polling production smoke 验证 `/devices/device-relay-01` 桌面 `1366x900`、移动端 `390x844` 无横向溢出，`delivered` lifecycle summary 的 `data-status-tone` 为 `inflight`；build 后已删除 `apps/web/.next/`
- 2026-06-09 设置页危险操作确认复核：`npm test -- --run apps/web/src/app/settings/settings-page.test.tsx` 1 file / 1 test 通过；随后全量 `npm test -- --run` 25 files / 114 tests 通过；`npm run lint` 和 `npm run build` 通过；production smoke 验证 `/settings` 桌面 `1366x900`、移动端 `390x844` 均显示应用内“恢复出厂设置”对话框，不触发原生 browser dialog，确认后主灯继电器变为“离线 / 未分配”，无横向溢出；build 后已删除 `apps/web/.next/`
- Playwright QA:
