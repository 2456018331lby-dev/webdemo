...
- Settings center with local preferences, notification rules, notification inbox, push readiness checks, local backup export/restore, per-key restore validation plus risk preview for known local state, persisted device maintenance state, and a reset path back to the default device list
...
- 2026-06-09 local backup restore preview UX: targeted `settings-page.test.tsx` and `local-app-backup.test.ts` passed; full `npm test -- --run --maxWorkers=1` passed, 26 files / 125 tests; `npm run lint` passed; `npm run build` passed; no-output Playwright smoke verified `/settings` backup risk preview at `1366x900` and `390x844` with no horizontal overflow. Restore preview now separates restorable, overwrite, invalid, redacted, unknown, and missing counts before applying a backup; `apps/web/.next/` was deleted after build/smoke.
