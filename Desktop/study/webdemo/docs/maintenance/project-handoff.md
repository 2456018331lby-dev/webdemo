...
- Settings center with local preferences, notification rules, notification inbox, push readiness checks, local backup export/restore, and persisted device maintenance state
...
2. Add a UI affordance for clearing locally customized settings-device state back to seed devices, if operators need a reset path separate from factory reset.
